...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<!--collection-id: gets collection-id from the first instance of fits/fileinfo/filepath in the FITS XML when the collection-id is formatted harg-ms####-->
	<xsl:template name="get-collection-id">
		<xsl:analyze-string select="(//fits:fileinfo/fits:filepath)[1]" regex="harg-ms\d{{4}}">
			<xsl:matching-substring><xsl:sequence select="."/></xsl:matching-substring>
		</xsl:analyze-string>
	</xsl:template>
	
	<!--aip-id: gets aip-id from the first instance of fits/fileinfo/filepath in the FITS XML when the aip-id is formatted harg-ms####er####-->
	<xsl:template name="get-aip-id">
		<xsl:analyze-string select="(//fits:fileinfo/fits:filepath)[1]" regex="harg-ms\d{{4}}er\d{{4}}">
			<xsl:matching-substring><xsl:sequence select="."/></xsl:matching-substring>
		</xsl:analyze-string>
	</xsl:template>
	
	<!--aip title: gets aip title from the first instance of fits/fileinfo/filepath in the FITS XML when the filepath is formatted any/thing/harg-ms####er####_AIP Title/anything/else.ext-->
	<xsl:template name="get-aip-title">
		<xsl:analyze-string select="(//fits:fileinfo/fits:filepath)[1]" regex="harg-ms\d{{4}}er\d{{4}}_(.*?)/">
			<xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring>
		</xsl:analyze-string>
	</xsl:template>
	
	<!--file-id: gets relative file path (everything beginning with aip-id) from the first instance of fits/fileinfo/filepath in the FITS XML when the aip-id is formated harg-ms####er####-->
	<xsl:template name="get-file-id">
		<xsl:analyze-string select="fits:fileinfo/fits:filepath" regex="harg-ms\d{{4}}er\d{{4}}.*">
			<xsl:matching-substring><xsl:sequence select="."/></xsl:matching-substring>
		</xsl:analyze-string>
	</xsl:template>
